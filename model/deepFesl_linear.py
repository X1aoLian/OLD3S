...
class deepFESL_linear:
    def __init__(self, data_S1, label_S1, data_S2, label_S2, T1, t, b=0.9, lr=0.01, s=0.008, m=0.95, spike=9e-5,
                 thre=10000):
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        
        # self.conv1=nn.Conv2d(1,3,kernel_size=3,stride=1, padding=1, bias=False).to(self.device)
        # self.Tconv1=nn.ConvTranspose2d(3, 1, kernel_size=3, stride=1, padding=1, bias=False).to(self.device)
    ...
    def FESA(self):  # Feature Evolving Streaming Approximate
        "load data"
        ...
        self.CELoss = nn.CrossEntropyLoss()
        self.BCEloss = nn.BCELoss()
        self.SmoothL1Loss = nn.SmoothL1Loss()  # L1-distance to measure the similarity of two latent representations
        ...
        for (i, x) in enumerate(data1):
            ...
            if self.i < self.B:  # Before evolve
                ...
                loss_1, y_hat = self.HB_Fit(self.net_model1, x1, y, optimizer_1)
                ...
                if i == self.B:
                    ...
                    optimizer_map = torch.optim.SGD(self.fn.parameters(), lr = 0.08)
                ...
                loss_1_1, y_hat_1 = self.HB_Fit(self.net_model1, x1, y, optimizer_1_1)
                loss_1_2, y_hat_2 = self.HB_Fit(self.net_model2, x2, y, optimizer_1_2)
        ...
        torch.save(self.net_model2.state_dict(), '/home/hlian001/fesdl/data/parameter_hedge/net_model2_linear.pth')
    ...
    def FESDL(self):
        ...
        pretrain_dict = torch.load('/home/hlian001/fesdl/data/parameter_hedge/net_model1_linear.pth')    # One model, no ensembling
        ...
        pretrain_dict = torch.load('/home/hlian001/fesdl/data/parameter_hedge/net_model2_linear.pth')    # One model, no ensembling
        ...
        for (i, x) in enumerate(data2):
            ...
            x1_new = (self.fn(x2_flatten)).reshape(3,32,32).unsqueeze(0).to(self.device)

            loss_4, y_hat_2 = self.HB_Fit(net_model2, x, y, optimizer_4)
            loss_3, y_hat_1 = self.HB_Fit(net_model1, x1_new, y, optimizer_3)
        ...
        torch.save(self.S1_Figure_Accuracy, '/home/hlian001/fesdl/data/deepFESL_linear_accuracy')
    ...
    def classifier(self, model, X, Y, block_split=6):
        predictions_per_layer = model.forward(X)
        output = torch.empty_like(predictions_per_layer[0])
        for i, out in enumerate(predictions_per_layer):
            output += self.alpha1[i] * out
